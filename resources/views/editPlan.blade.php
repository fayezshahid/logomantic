...
<style>
    ...
    <div class="row d-flex justify-content-center">
        <div class="col-8" style="margin-top: 30px;">
            <form action="{{ route('editPlan', $plan->id) }}" method="post" id="form">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label class="form-label" style="font-weight: normal">Plan Name</label>
                    <input type="text" name="name" placeholder="Enter plan name" class="form-control @error('name') border-3 border-danger @enderror" value="{{ $plan->name }}">
                    @error('name')
                ...
                <div class="mb-4">
                    <label class="form-label" style="font-weight: normal">Plan Price</label>
                    <input type="text" name="price" placeholder="Enter plan price" class="form-control @error('price') border-3 border-danger @enderror" value="{{ $plan->price }}">
                    @error('price')
                        <div class="text-danger">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-4" id="itemDiv">
                    <label class="form-label" style="font-weight: normal">Plan items</label>
                    <?php $i = 0 ?>
